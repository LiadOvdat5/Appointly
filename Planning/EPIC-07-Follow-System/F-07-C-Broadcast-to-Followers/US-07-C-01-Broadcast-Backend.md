...
## Acceptance Criteria
- [x] Only the business owner can send a broadcast to their followers
- [x] A business cannot send more than 3 broadcasts in any rolling 24-hour window; returns `429 Too Many Requests`
- [x] The HTTP request returns `202` before fan-out completes — fan-out is async
- [x] Each follower receives exactly one in-app notification per broadcast (no duplicates on retry)
- [x] Broadcast history is stored and retrievable by the owner
- [x] Title and body length are validated; returns `400` if exceeded
