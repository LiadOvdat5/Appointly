...
## Acceptance Criteria
- [x] Customer receives exactly one reminder notification per appointment (no duplicates)
- [x] Reminder is sent approximately 24 hours before the appointment time
- [x] Cancelled appointments do not trigger reminders
- [x] The background job is resilient — if it fails once, it retries on the next run without double-sending
